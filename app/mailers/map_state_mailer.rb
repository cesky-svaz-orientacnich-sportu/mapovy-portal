...
class MapStateMailer < ActionMailer::Base
  ...
  def map_changed_after_request(map, region_was)
    ...
    mail(to: map.authorized_cartographers(region_was).map(&:email), cc: User.where(role: 'admin').map(&:email), subject: "Doplněný požadavek na evidenci mapy #{@map}")
  end

  def map_approved(map)
    @map = map
    mail(to: map.created_by.email, cc: User.where(role: 'admin').map(&:email), subject: "Požadavek na evidenci mapy #{@map} přijat")
  end
  ...
  def reminder_on_proposed(map)
    ...
    mail(to: map.authorized_cartographers.map(&:email), cc: User.where(role: 'admin').map(&:email), subject: "Připomínka -- čekající požadavek na evidenci mapy #{@map}")
  end
  ...
  def reminder_on_requested_change(map)
    ...
    @map = map
    map.update_attribute :last_reminder_sent_at, Date.today
    mail(to: map.created_by.email, cc: User.where(role: 'admin').map(&:email), subject: "Připomínka -- je potřeba doplnit údaje k mapě #{@map}")
  end
